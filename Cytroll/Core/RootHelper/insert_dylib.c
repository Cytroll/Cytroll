/*
 * insert_dylib — Mach-O LC_LOAD_DYLIB(/LC_LOAD_WEAK_DYLIB) injector.
 *
 * Vendored (verbatim, minus SDK portability tweaks) from:
 *   https://github.com/Tyilo/insert_dylib
 * Original author: Tyilo. No formal LICENSE file is published upstream;
 * the tool is widely redistributed as source across the jailbreak
 * ecosystem (including by TrollFools itself) for exactly this purpose —
 * patching third-party app binaries to load an injected tweak dylib.
 *
 * Cytroll compiles this from source (same pattern as cytrollhelper.c)
 * instead of fetching a precompiled binary, so the exact bytes running
 * on-device are auditable from this repo. Pure Mach-O byte manipulation —
 * no host-OS-specific behavior, so it cross-compiles cleanly for
 * `-sdk iphoneos -arch arm64` exactly like the macOS build.
 *
 * Usage: insert_dylib dylib_path binary_path [new_binary_path]
 * Option flags: --inplace --weak --overwrite --strip-codesig
 *               --no-strip-codesig --all-yes
 */
